...
if __name__ == '__main__':
    ...
    input_delivery_appointment_date = 'delivery appointment date'
    input_delivery_appointment_time = 'delivery appointment time'

    load_numbers = Input.get_general_input(input_load_number)
    delivery_appointment_date = Input.get_date(input_delivery_appointment_date)
    delivery_appointment_time = Input.get_time(input_delivery_appointment_time)

    # START COUNTING RUNNING TIME
    start = time.time()

    print('Delivery appointment', delivery_appointment_date, delivery_appointment_time, 'will be added to load(s):',
          load_numbers, '.')
    ...
    print('Adding delivery appointment...')
    ...
    for loads in load_number_list:
        ...
        transport_report_data_dict = Config_Post_Data.config_transport_delivery_appt(csrf, concatenated_loads)

        Get_Data.get_transport_report_by_report_format(session_requests, transport_report_data_dict)

        # ADD PU APPT
        appt_update_data_dict = Config_Post_Data.config_update_delivery_appt(csrf, load_numbers=loads,
                                                                             appt_date=delivery_appointment_date,
                                                                             appt_time=delivery_appointment_time)
    ...
    print('Delivery appointment has been successfully added.')
    time.sleep(10)
